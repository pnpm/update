...
| `update-pnpm` | pinned major | Bump pnpm itself via `pnpm self-update`. Defaults to the latest release of the currently pinned major; set a version, range, or dist-tag (`latest`, `12`, `next-12`) to move onto it, or `false` to skip. The pnpm bump runs before everything else, so the dependency update and the lockfile are produced by the new pnpm version. |
